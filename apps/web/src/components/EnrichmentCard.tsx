...
export function EnrichmentCard({ enrichment }: { enrichment: EnrichmentResult }) {
  return (
    <div className="rounded-lg border border-border bg-card p-4 space-y-2">
      ...
      <p className="text-sm text-muted">{enrichment.description ?? "No description available."}</p>
      <div className="flex flex-wrap gap-2 text-xs">
        {enrichment.industry && (
          <span className="rounded bg-border px-2 py-1">{enrichment.industry}</span>
        )}
        {enrichment.employee_band && (
          <span className="rounded bg-border px-2 py-1">{enrichment.employee_band}</span>
        )}
        {enrichment.domain && (
